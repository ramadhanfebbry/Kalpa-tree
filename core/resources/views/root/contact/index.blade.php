...
@section('description', 'Contact Us Management')
...
@section('content')
    ...
    <table class="table table-hover" id="contact-table">
        <thead>
            <tr>
                <th> Name </th><th> Email </th><th> Phone </th><th width="25%"> Messages </th>
            </tr>
        </thead>
    </table>
...
var oTable;
oTable = $('#contact-table').DataTable({
    processing: true,
    serverSide: true,
    dom: 'lBfrtip',
    order:  [[ 0, "asc" ]],
...
$("#contact-table_wrapper > .dt-buttons").appendTo("div.export-options-container");
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("root/contact")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
