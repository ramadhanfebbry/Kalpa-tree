...
@section('description', 'Article Management')
@section('breadcrumbs', Breadcrumbs::render('article'))
@section('button', '<a href="'.url('/root').'/article/create'.'" class="btn btn-primary btn-xs no-border">Add Data <i class="fa fa-plus"></i></a>')
...
@section('content')
    ...
    <table class="table table-hover" id="article-table">
        <thead>
            <tr>
                <th> Image </th><th> Title </th><th> Descriptions </th><th width="15%"> Actions </th>
            </tr>
        </thead>
    </table>
...
var oTable;
oTable = $('#article-table').DataTable({
    processing: true,
    serverSide: true,
    dom: 'lBfrtip',
    order:  [[ 0, "asc" ]],
...
$("#article-table_wrapper > .dt-buttons").appendTo("div.export-options-container");
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("root/article")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
