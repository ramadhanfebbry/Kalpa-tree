...
@section('description', 'Main Dishes Management')
@section('breadcrumbs', Breadcrumbs::render('main-dishes'))
...
@section('content')
    ...
    <table class="table table-hover" id="main-dishes-table">
        <thead>
            <tr>
                <th width="15%"> Title </th><th width="25%"> Intro </th><th width="45%"> Descriptions </th><th width="15%"> Actions </th>
            </tr>
        </thead>
    </table>
...
var oTable;
oTable = $('#main-dishes-table').DataTable({
    processing: true,
    serverSide: true,
    dom: 'lBfrtip',
    order:  [[ 0, "asc" ]],
...
$("#main-dishes-table_wrapper > .dt-buttons").appendTo("div.export-options-container");
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("root/main-dishes")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
