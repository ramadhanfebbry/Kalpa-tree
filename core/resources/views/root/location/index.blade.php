...
@section('breadcrumbs', Breadcrumbs::render('location'))
...
@section('content')
    ...
    <table class="table table-hover" id="location-table">
        <thead>
            <tr>
                <th width="25%"> Title </th><th> Descriptions </th><th width="15%"> Actions </th>
            </tr>
        </thead>
    </table>
...
var oTable;
oTable = $('#location-table').DataTable({
    processing: true,
    serverSide: true,
    dom: 'lBfrtip',
    order:  [[ 0, "asc" ]],
...
$("#location-table_wrapper > .dt-buttons").appendTo("div.export-options-container");
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("root/location")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
