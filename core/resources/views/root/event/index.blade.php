...
@section('title', 'Event')
@section('description', 'Event Management')
@section('breadcrumbs', Breadcrumbs::render('event'))
@section('button', '<a href="'.url('/root').'/event/create'.'" class="btn btn-primary btn-xs no-border">Add Data <i class="fa fa-plus"></i></a>')
...
@section('content')
    ...
    <table class="table table-hover" id="event-table">
        <thead>
            <tr>
                <th> Image </th><th> Title </th><th> Descriptions </th><th> Order </th><th width="15%"> Actions </th>
            </tr>
        </thead>
    </table>
...
$("#event-table_wrapper > .dt-buttons").appendTo("div.export-options-container");
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("root/event")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
