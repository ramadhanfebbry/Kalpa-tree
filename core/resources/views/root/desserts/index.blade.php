...
@section('description', 'Desserts Management')
@section('breadcrumbs', Breadcrumbs::render('desserts'))
...
@section('content')
    ...
    <table class="table table-hover" id="desserts-table">
        <thead>
            <tr>
...
var oTable;
oTable = $('#desserts-table').DataTable({
    processing: true,
    serverSide: true,
    dom: 'lBfrtip',
    order:  [[ 0, "asc" ]],
...
$("#desserts-table_wrapper > .dt-buttons").appendTo("div.export-options-container");
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("root/desserts")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
