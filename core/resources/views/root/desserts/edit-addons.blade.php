...
@section('breadcrumbs', Breadcrumbs::render('desserts.edit',$addons))
@section('button', '<a href="'.url('/root/desserts').'/'.$desserts->id.'" class="btn btn-info btn-xs no-border">Back
</a>')

@section('content')
    {!! Form::open(['url' => '/root/desserts-addons/'.$addons->id.'/update', 'id' => 'formValidate', 'files' => true]) !!}
    @if($desserts->addons == "images")
    <div aria-required="true" class="form-group form-group-default {{ $errors->has('images') ? 'has-error' : ''}}">
        ...
        @if(isset($addons->image))
            <img src="{{url('').'/files/desserts/'.$addons->image}}" style="max-width:200px; margin:10px 0px;" />
        @endif
