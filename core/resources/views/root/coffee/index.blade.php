...
@section('description', 'Coffee Management')
@section('breadcrumbs', Breadcrumbs::render('gallery'))
@section('button', '<a href="'.url('/root').'/coffee/create'.'" class="btn btn-primary btn-xs no-border">Add Data <i class="fa fa-plus"></i></a>')
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("root/coffee")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
