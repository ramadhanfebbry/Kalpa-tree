...
@section('description', 'Drinks Management')
@section('breadcrumbs', Breadcrumbs::render('drinks'))
...
@section('content')
    ...
    <table class="table table-hover" id="drinks-table">
        <thead>
            <tr>
...
$("#drinks-table_wrapper > .dt-buttons").appendTo("div.export-options-container");
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("root/drinks")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
