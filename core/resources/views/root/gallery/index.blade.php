...
@section('description', 'Gallery Management')
@section('breadcrumbs', Breadcrumbs::render('gallery'))
@section('button', '<a href="'.url('/root').'/gallery/create'.'" class="btn btn-primary btn-xs no-border">Add Data <i class="fa fa-plus"></i></a>')
...
@section('content')
    ...
    <table class="table table-hover" id="gallery-table">
        <thead>
            <tr>
                <th> Image </th><th> Title </th><th width="15%"> Actions </th>
            </tr>
        </thead>
    </table>
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("root/gallery")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
