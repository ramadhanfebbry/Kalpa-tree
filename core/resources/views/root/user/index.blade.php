...
@section('button', '<a href="'.url('/root').'/'.Request::segment(2).'/create'.'" class="btn btn-primary btn-xs no-border">Add Data <i class="fa fa-plus"></i>
</a> ')
...
@section('content')
    ...
    <table class="table table-hover" id="user-table">
        <thead>
            <tr>
                <th> Name </th><th> Username </th><th> Email </th><th width="15%"> Actions </th>
            </tr>
        </thead>
    </table>
...
oTable = $('#user-table').DataTable({
    ...
    ajax: {
    url: 
    @if(Request::segment(2)!='user-admin')
        '{!! route('user.data') !!}'
    @else
        '{!! route('user-admin.data') !!}'   
    @endif
    ,
        data: function (d) {
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("root/user")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
