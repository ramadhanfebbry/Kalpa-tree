...
@section('description', 'Appetizer Management')
@section('breadcrumbs', Breadcrumbs::render('appetizer'))
...
@section('content')
    ...
    <table class="table table-hover" id="appetizer-table">
        <thead>
            <tr>
...
var oTable;
oTable = $('#appetizer-table').DataTable({
    processing: true,
    serverSide: true,
    dom: 'lBfrtip',
    order:  [[ 0, "asc" ]],
...
$("#appetizer-table_wrapper > .dt-buttons").appendTo("div.export-options-container");
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("root/appetizer")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
