...
@section('breadcrumbs', Breadcrumbs::render('appetizer.edit',$addons))
@section('button', '<a href="'.url('/root/appetizer').'/'.$appetizer->id.'" class="btn btn-info btn-xs no-border">Back
</a>')

@section('content')
    {!! Form::open(['url' => '/root/appetizer-addons/'.$addons->id.'/update', 'id' => 'formValidate', 'files' => true]) !!}
    @if($appetizer->addons == "images")
    <div aria-required="true" class="form-group form-group-default {{ $errors->has('images') ? 'has-error' : ''}}">
        ...
        @if(isset($addons->image))
            <img src="{{url('').'/files/appetizer/'.$addons->image}}" style="max-width:200px; margin:10px 0px;" />
        @endif
