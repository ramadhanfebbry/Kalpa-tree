...
@section('description', 'Slider Management')
@section('breadcrumbs', Breadcrumbs::render('slider'))
@section('button', '<a href="'.url('/root').'/slider/create'.'" class="btn btn-primary btn-xs no-border">Add Data <i class="fa fa-plus"></i></a>')
...
@section('content')
    ...
    <table class="table table-hover" id="slider-table">
        <thead>
            <tr>
                <th> Image </th><th> Tagline </th><th> Order </th><th width="15%"> Actions </th>
            </tr>
        </thead>
    </table>
...
$("#slider-table_wrapper > .dt-buttons").appendTo("div.export-options-container");
...
function hapus(){
    ...
    var id = $('#did').val();
    $.ajax({
        url: '{{url("root/slider")}}' + "/" + id + '?' + $.param({"_token" : '{{ csrf_token() }}' }),
        type: 'DELETE',
